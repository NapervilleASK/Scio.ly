...
def list_pdf_files_in_folder(service, folder_id):
    ...
    try:
        ...
        subfolder_results = service.files().list(q=subfolder_query, fields="nextPageToken, files(id)").execute()
        ...
        while nextPageToken:
            results = service.files().list(q=query, fields="nextPageToken, files(id, name)", pageToken=nextPageToken).execute()
            items = results.get('files', [])
            pdf_files.extend(items)
            nextPageToken = results.get('nextPageToken')
...
def extract_questions_with_gemini(client, text, events, idx):
    ...
    Extract all multiple-choice questions and output only a compressed json object of this test using this schema:
    ...
    IF THE QUESTION MENTIONS AN IMAGE OR DIAGRAM, DO NOT INCLUDE IT. 
    Multiple choice answers MUST be 1-indexed, and usually have only one correct answer but sometimes multiple.
    Anatomy sections may have multiple systems, so you can use multiple upper level keys.
    You may leave the option field blank for free response, but have one element in the answers array providing a correct response.
    Here's the test:
    {text}
...
def process_pdf(file_info, drive_service, gemini_model, events, gemini_api_keys, output_dir, idx_offset):
    idx = idx_offset
    file_name = file_info['name']
    file_id = file_info['id']

    if not (file_name.lower().endswith('.pdf') and (re.search(r"test|exam", file_name.lower()) or not re.search(r"key|sheet|answer|result|image|response", file_name.lower()))):
    ...
    if gemini_output is None:
        return {}
    try:
        questions_json_str = re.search(r"\{.*\}", gemini_output, re.DOTALL)
        if questions_json_str:
            questions_json = json.dumps(json.loads(questions_json_str.group(0)))
...
with open("beta_bank.json", 'a') as writefile:
    for folder_id in folder_links:
        ...
        for i in range(0, len(pdf_files_in_folder), 10):
            batch_files = pdf_files_in_folder[i:i + 10]
            batch_results = Parallel(n_jobs=10)(
                delayed(process_pdf)(file_info, drive_service, gemini_model, events, GEMINI_API_KEY, OUTPUT_DIR, idx + j)
