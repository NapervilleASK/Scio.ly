...
def list_pdf_files_in_folder(service, folder_id):
    ...
    pdf_files = []
    try:
        results = service.files().list(supportsAllDrives=True, includeItemsFromAllDrives=True, q=f"'{folder_id}' in parents  and mimeType='application/pdf' and trashed=false", fields="nextPageToken, files(id, name)").execute()
        ...
        while subfolder_nextPageToken:
            subfolder_results = service.files().list(supportsAllDrives=True, includeItemsFromAllDrives=True, q=subfolder_query, fields="nextPageToken, files(id)", pageToken=subfolder_nextPageToken).execute()
            subfolders = subfolder_results.get('files', [])
            for subfolder in subfolders:
                pdf_files.extend(list_pdf_files_in_folder(service, subfolder['id']))
            subfolder_nextPageToken = subfolder_results.get('nextPageToken')
...
def process_pdf(file_info, drive_service, events, output_dir, idx_offset):
    ...
    if not file_name.lower().endswith('.pdf') or re.search(r"experiment|forestry|widi|write it|tower|scrambler|robot|trajectory|helicopter|electric|bungee", file_name.lower()) or re.search(r"key|sheet|answer|result|image|response|checklist|distribution", file_name.lower()):
    ...
    raw_output = extract_questions_with_gemini(text_content, events, idx).replace('\00','f[]')
    if raw_output is None:
        return {}
    try:
        gemini_output = clean_question_with_gemini(raw_output,idx).replace('\00','f[]')
...
with open("beta_bank.json", 'a') as writefile:
    for folder_id in folder_links:
        ...
        pdf_files_in_folder = [f for f in all_files_in_folder if 'name' in f and f['name'].lower().endswith('.pdf')]
        ...
        for i in range(0, len(pdf_files_in_folder), 7):
            batch_files = pdf_files_in_folder[i:i + 7]
            batch_results = Parallel(n_jobs=7)(
