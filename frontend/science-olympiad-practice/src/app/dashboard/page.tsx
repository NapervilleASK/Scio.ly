...
const events: Event[] = [ // if not in indexedDB or broswer doesn't support indexedDB, fetch(https://raw.githubusercontent.com/NapervilleASK/SciOly_scraper/refs/heads/main/jsonminifier.json?token=GHSAT0AAAAAAC3JKEFQK6XT6ID4XW7VUEMKZ4O3JBQ)
                          // save to indexedDB if applicable
                          // Populate event options with the upper level JSON keys
...
function EventDashboard() {
  ...
  const handleChange = (e: { target: { id: any; value: any; }; }) => {
    ...
    setSettings((prev) => ({
      ...prev,
      [id]: value, // update state based on the input's id and value
    }));
  ...
  const handleGenerateTest = () => {
    ...
    router.push(
      `/test?eventName=${encodeURIComponent(selectedEventDetails.name)}&questionCount=${settings.questionCount}&timeLimit=${settings.timeLimit}&difficulty=${settings.difficulty}&types=${settings.types}`
    );
  };
            ...
            {sortedEvents.map((event) => (
              ...
              >
                <div className="flex justify-between items-center">
                  <h2 className="text-black text-lg font-semibold">{event.name}</h2>
